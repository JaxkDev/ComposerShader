...
/*
 * TODO cases like this: (fortunately i dont know a single plugin or composer lib that does this)
 * namespace z\y\x
 * //comment
 * {
 *	...
 ...
 * const COMPOSER_AUTOLOAD = __DIR__.'/TDFADSADSA/autoload.php';
...
function injectCode(string $content, string $code): string{
	...
	foreach($tokens as $token){
		if(is_array($token)){
			...
			if($id === T_NAMESPACE){
				$namespaceLine = $line;
			} elseif($namespaceLine !== null and $line > $namespaceLine){
				$str = "\n//The following code is generated by ComposerShader, do not touch.\n$code\n//End of generated code.\n\n" . $str;
				$namespaceLine = null;
			}
			$ret .= $str;
